...
var hasActivity = function (line) {
    ...
    words.forEach(function (word) {
        if (word.match(/^\(/)) {
            comment = true;
        } else if (!comment && (word.search(TIMEUNITS) === 0 || word.search(/[^0-9Xx:]/) > -1)) {
            result = true;
        } else if (word.match(/\)$/)) {
            comment = false;
        }
...
module.exports = function parse(data) {
    var currentActivity, currentLine, currentSet, currentWord, lastType, lines, pr, setIndex, sets, words;
    var currentIndex = -1;
    ...
    while (lines.length > 0) {
        ...
        while (words.length > 0) {
            ...
            if (currentWord.match(COMMENTSTART) && currentIndex > -1) {
                ...
                words.shift();
                lastType = 'distance';
            } else if (currentWord.match(LIFTEXPR)) {
                if (currentWord.match(PREXPR)) {
                    pr = true;
                    currentWord = currentWord.slice(0, -1);
                } else {
                    ...
                    if (set[1]) {
                        if (set[0].match(WEIGHTEXPR)) {
                            if (set[0].toLowerCase().match(/lb/)) {
                                currentActivity.sets.push({weight: Number(set[0].replace(/lb/, '')), reps: Number(set[1]), unit: 'kg'});
                                ...
                                currentActivity.sets.push({weight: Number(set[0]), reps: Number(set[1]), unit: 'lb'});
                            } else {
                                currentActivity.sets.push({reps: Number(set[1])});
                            }
                        } else {
                            currentActivity.sets.push({reps: Number(set[1]), unit: 'lb'});
                        }
                    } else {
